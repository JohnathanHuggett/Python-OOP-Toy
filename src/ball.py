...
class Ball:
    ...
    def update(self):
        ...
        if self.position.x < 0 + self.radius or self.position.x > self.bounds[0] - self.radius:
            self.velocity.x *= -1
        # screen height
        if self.position.y + self.radius > self.bounds[1] or self.position.y < 0 + self.radius:
            self.velocity.y *= -1
...
class BouncingBall(Ball):
    ...
    def update(self):
        if self.position.y + self.radius + self.velocity.y > self.bounds[1] - .09 * self.radius:
            self.velocity.y *= -1
        ...
        if self.position.x - self.radius >= self.bounds[0]:
